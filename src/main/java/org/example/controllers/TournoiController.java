...
public class TournoiController implements Initializable {
    ...
    private void handleNavCentres() {
        try { MainFX.navigateTo("CentreView.fxml", "🏢  Centre Manager"); }
    ...
    private String validate(TextField tfNom, ComboBox<String> cbJeu,
        ...
        if (tfNom.getText().trim().isEmpty())  err.append("• Le nom est requis.\n");
        if (cbJeu.getValue() == null)          err.append("• Le jeu est requis.\n");
        if (dpDebut.getValue() == null)        err.append("• La date de début est requise.\n");
        if (dpFin.getValue() == null)          err.append("• La date de fin est requise.\n");
        ...
        if (tfLieu.getText().trim().isEmpty())  err.append("• Le lieu est requis.\n");
        try {
            double v = Double.parseDouble(tfPrix.getText().trim().replace(",", "."));
            if (v < 0) err.append("• Le prix doit être positif.\n");
        } catch (NumberFormatException e) { err.append("• Le prix doit être un nombre valide.\n"); }
        return err.toString();
    }
