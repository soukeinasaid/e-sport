...
public class FrontOfficeController implements Initializable {
    ...
    private void loadCentres() {
        try { centreList = centreService.listerTous(); }
        catch (SQLException e) { showError("Centres", e.getMessage()); }
    }
    ...
    private void populateGameFilter() {
        ...
        allTournois.stream()
            .map(Tournoi::getJeu).distinct().sorted()
    ...
    private void applyFilter() {
        ...
        List<Tournoi> filtered = allTournois.stream()
            ...
            .filter(t -> game == null || game.equals("Tous les jeux")
                    || t.getJeu().equals(game))
            .toList();
    ...
    private VBox buildCard(Tournoi t, int colorIdx) {
        ...
        if (t.getDateDebut().isAfter(today)) {
            status = "🔵  À venir"; statusClass = "fo-status-upcoming";
        } else if (t.getDateFin().isBefore(today)) {
            status = "⚫  Terminé";  statusClass = "fo-status-finished";
        } else {
            status = "🟢  En cours"; statusClass = "fo-status-active";
        }

        // Centre name
        String centreName = centreList != null ? centreList.stream().filter(c -> c.getId() == t.getCentreId()).findFirst().map(Centre::getName).orElse("—") : "—";
    ...
    private void updateStats(List<Tournoi> list) {
        LocalDate today = LocalDate.now();
        long active   = list.stream().filter(t -> !t.getDateDebut().isAfter(today) && !t.getDateFin().isBefore(today)).count();
        long upcoming = list.stream().filter(t -> t.getDateDebut().isAfter(today)).count();
    ...
    private void showDetails(Tournoi t, String centreName) {
        ...
        dlg.setTitle("Détails du Tournoi"); dlg.setHeaderText(null);

        VBox c = new VBox(12);
        c.setPadding(new Insets(24)); c.setStyle("-fx-background-color:#181C28;"); c.setPrefWidth(400);
        ...
        c.getChildren().addAll(title, badge, sep,
        ...
        // Add buttons based on tournament status
        LocalDate today = LocalDate.now();
    ...
    private HBox dRow(String label, String value) {
        Label lbl = new Label(label); lbl.setStyle("-fx-text-fill:#8A93A5;-fx-font-size:12px;-fx-min-width:110;");
        Label val = new Label(value); val.setStyle("-fx-text-fill:#FFF;-fx-font-size:13px;-fx-font-weight:600;");
    ...
    private void handleParticipate(Tournoi t) {
        currentTournament = t;
        try {
            // Navigate to team selection view, passing the tournament
            MainFX.navigateTo("ParticipationView.fxml", "🏆  Participation — " + t.getNom());
        } catch (Exception e) {
    ...
    private void handleNavCentres() {
        try { MainFX.navigateTo("FrontOfficeCentreView.fxml", "🏢  Front Office — Centres"); }
    ...
    private void handleViewBracket(Tournoi t) {
        currentTournament = t;
        try {
            MainFX.navigateTo("BracketView.fxml", "📊  Bracket — " + t.getNom());
        } catch (Exception e) {
            showError("Navigation", e.getMessage());
        }
    }
    ...
    private void handleViewLeaderboard(Tournoi t) {
        currentTournament = t;
        try {
            MainFX.navigateTo("LeaderboardView.fxml", "📊  Classements & Statistiques — " + t.getNom());
        } catch (Exception e) {
            showError("Navigation", e.getMessage());
        }
    }
