...
public class FrontOfficeCentreController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        if (Session.getUser() != null) {
            String name = Session.getUser().getNom();
        }
        loadData();
    ...
    private void handleNavTournois() {
        try { MainApp.navigateTo("FrontOfficeView.fxml", "🏆  Front Office — Tournois"); }
    ...
    private void handleLogout() {
      //  SessionManager.logout();
        try { MainApp.navigateTo("login.fxml", "⚔  TournoiManager — Connexion"); }
