...
public class LoginController {
    ...
    @FXML
    private void handleFacebookSignIn(ActionEvent event) {
        try {
            // Show information dialog about Facebook sign-in
            Alert infoAlert = new Alert(Alert.AlertType.INFORMATION);
            infoAlert.setTitle("Facebook Sign-In");
            infoAlert.setHeaderText("Facebook Authentication");
            infoAlert.setContentText("Facebook OAuth2 integration requires additional setup.\n\nFor now, please use the regular login with your email and password, or create a new account.");
            infoAlert.showAndWait();
            
        } catch (Exception e) {
            showError("Facebook sign-in failed: " + e.getMessage());
            e.printStackTrace();
        }
    }
